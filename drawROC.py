...
for i, data in enumerate(testloader, 0):  # 这里我们遇到了第一步中出现的trailoader，代码传入数据
    ...
    outputs_fromCNN = model(Variable(oct),Variable(octa))
...
lableforconfusion=np.array(lableforconfusion)
# 计算每一类的ROC
fpr = dict()
tpr = dict()
roc_auc = dict()
...
plt.savefig('./savedimg/confusionmatrix.png', dpi=300, bbox_inches='tight')
plt.show()
...
plt.savefig('./savedimg/ROC.png', dpi=300)
plt.show()